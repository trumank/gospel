...
impl CodeGenerationContext {
    ...
    fn generate_type_field_definition(&self, type_name: &Ident, source_file_name: &str, field_name: &Ident, maybe_field_type_index: Option<usize>, is_prototype_field: bool, type_definition: &BindingsTypeDefinition) -> TokenStream {
        ...
        if let Some(field_type_index) = maybe_field_type_index && !self.is_opaque_type_index(field_type_index) {
            ...
            if is_prototype_field {
                quote! { gsb_codegen_implement_udt_field!(#type_name, #field_name, #source_file_name, optional, { #field_doc_comment }, #field_type); }
            } else {
                quote! { gsb_codegen_implement_udt_field!(#type_name, #field_name, #source_file_name, required, { #field_doc_comment }, #field_type); }
            }
        } else {
            // We cannot generate an accurate field type, so just return DynamicPtr as-is
            if is_prototype_field {
                quote! { gsb_codegen_implement_udt_field!(#type_name, #field_name, #source_file_name, optional, { #field_doc_comment }); }
            } else {
                quote! { gsb_codegen_implement_udt_field!(#type_name, #field_name, #source_file_name, required, { #field_doc_comment }); }
            }
        }
    }
    fn generate_udt_definition(&self, user_defined_type: &UserDefinedType, type_container: &GospelVMTypeContainer, type_definition: &BindingsTypeDefinition) -> TokenStream {
        ...
        for udt_member in &user_defined_type.members {
            if let UserDefinedTypeMember::Field(field) = udt_member && let Some(field_name) = field.name.as_ref() && !field_name.contains("@") {
                let field_tokens = self.generate_type_field_definition(&type_name, field_name, &Ident::new(&Self::convert_field_name_to_snake_case(field_name), Span::call_site()),
        ...
        quote! {
            ...
            gsb_codegen_implement_udt_type!(#type_name, #full_type_name, #type_impl_arguments);
            impl #type_name {
                #(#generated_fields)*
            }
        }
    ...
    fn generate_enum_constant_definition(&self, enum_definition: &BindingsTypeDefinition, type_name: &Ident, enum_underlying_type: &Option<PrimitiveType>, source_constant_name: &str, constant_name: Ident, is_prototype_constant: bool) -> TokenStream {
        ...
        let type_impl_kind = if enum_definition.is_static_type { quote! { static_type } } else { quote!{ dynamic_type } };
    ...
    fn generate_enum_definition(&self, enum_type: &EnumType, type_container: &GospelVMTypeContainer, type_definition: &BindingsTypeDefinition) -> TokenStream {
        ...
        let type_impl_kind = if type_definition.is_static_type { quote! { static_type } } else { quote!{ dynamic_type } };
    ...
    pub(crate) fn generate_bindings_file(self) -> anyhow::Result<String> {
        ...
        let result_file_token_stream = quote! {
            #[macro_use(gsb_codegen_implement_udt_type, gsb_codegen_implement_udt_field, gsb_codegen_implement_enum_type, gsb_codegen_implement_enum_trivial_value, gsb_codegen_implement_enum_constant)]
            #[allow(unused)]
