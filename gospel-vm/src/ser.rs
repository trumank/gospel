...
impl<T: Sized + Readable> Readable for Option<T> {
    fn de<S: Read>(stream: &mut S) -> Result<Option<T>> {
        let has_value: bool = stream.de()?;
        if has_value {
            let value: T = stream.de()?;
            Ok(Some(value))
        } else {
            Ok(None)
        }
...
impl<T: Sized + Writeable> Writeable for Option<T> {
    fn ser<S: Write>(&self, stream: &mut S) -> Result<()> {
